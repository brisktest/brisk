...
function sendJobToMachine(job: string, machine: any) {
    machine.sendJob(job)
}
...
function args(files : string[] = []): string[] {
    return [...[
        ...
        //"npm run test"
    ] ,...(config.remoteEnv|| []).map((env: string) => `-e ${env}`) ,...[config.dockerContainer],...config.cmdArgs, ...files] 
}
...
const processes = Object.keys(config.hosts).map((host: string, index: number) => {
    const hostSpecificOverrides: { env: NodeJS.ProcessEnv } = {
        env: {
            DOCKER_HOST: host.toString(),
            CI_NODE_TOTAL: Object.keys(config.hosts).length.toString(),
            CI_NODE_INDEX: index.toString(),
            PATH: process.env.PATH,
            HOME: process.env.HOME,
        },
    }
    let files = splitFiles(Object.keys(config.hosts).length, index, workDir)
...
function configureOutput(
    ...
    processes: ChildProcessWithoutNullStreams[],jsonOutput: string[]
) {
